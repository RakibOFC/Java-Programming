...
class Circle extends TwoDimensionalShape {
    ...
    private final double circleArea = (Math.pow(getDimOne(), 2) * Math.PI);
...
class Square extends TwoDimensionalShape {
	...
	private final double squareArea = getDimOne() * getDimOne(); 
...
class Triangle extends TwoDimensionalShape {
	...
	private final double triangleArea = getDimOne() * getDimTwo() * 0.5;
...
class Tetrahedron extends ThreeDimensionalShape{
	...
	private final double tetrahedronArea = 4 * (0.5 * getDimOne() * getDimTwo()) ;
	private final double tetrahedronVolume = ((0.5 * getDimOne() * getDimTwo()) * getDimThree())/3;
...
class Cube extends ThreeDimensionalShape {
    ...
    private final double cubeArea = 6 * Math.pow(getDimOne(), 2.0);
    private final double cubeVolume = Math.pow(getDimOne(), 3.0);
...
class Sphere extends ThreeDimensionalShape {
	...
	private final double sphereArea = 4 * (Math.pow(getDimOne(), 2) * Math.PI);
	private final double sphereVolume = Math.PI * 4/3 * Math.pow(getDimOne(), 3.0);
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        shape[5] = new Tetrahedron(1.0, 2.0, 3.0);
